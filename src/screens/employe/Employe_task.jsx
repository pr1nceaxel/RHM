...
import { Select, DatePicker,Flex, Progress, Button, Input, Form, Modal, Table, Drawer, Space } from 'antd';
...
export function EmployeTask() {
  ...
  const columns = [
    {
      title: 'Employé/Équipe',
      dataIndex: 'employee',
      key: 'employee',
      // Filtre par employé/équipe
      filters: [...new Set(tasks.map(task => ({ text: task.employee, value: task.employee })))],
    ...
    },
    {
      ...
      key: 'description',
      // Limite la description à 30 caractères pour éviter les débordements
      render: (text) => (
    ...
    },
    {
      ...
      dataIndex: 'dates',
      key: 'dates',
      render: (dates) => `${dates[0].format('YYYY-MM-DD')} - ${dates[1].format('YYYY-MM-DD')}`,
    },
    {
      title: 'Priorité',
      dataIndex: 'priority',
  ...
  return (
    <div className=" mx-auto p-6">
      ...
      <Table
        dataSource={tasks.filter(task => task.employee.toLowerCase().includes(searchText.toLowerCase()))}
        columns={columns}
